...
# Compute the expansion of the orthonormal polynomial without e^(qm*x^m/2)
# nor a constant factor based on some heuristics.
function polyAsyRHgen(np, y, alpha, T::Int, qm, m::Int, UQ)
    if (qm == 1) && (m == 1)
        z = y/4/np
        ...
        if r == 1
            ...
            if mod(k,2) == 1
                for n = 0:mo
                    ...
                    WV[:,:,k,n+1] = -(-1)^(ceil(Int, k/2)+1)*(1im*sqrt(2))^k*(-2+0im)^(-k/2)/4^(k+1)*brac(k-1,alpha)*sum(repeat(reshape(g[k,1:(n+1) ], (1,1,n+1) ), outer=[2,2,1]).*Ts[:,:,(n+1):-1:1], dims=3)
                end
            else
                for n = 0:mo
                    Ts[:,:,n+1] = (alpha^2+k/2-1/4)/k*4*(n==0)*I  -2*(k-1/2)*[ OmE[n+1]   4^(-alpha)*1im*XiE[n+1]  ;   4^alpha*1im*ThE[n+1]   -OmE[n+1] ]
                    WV[:,:,k,n+1] = -(-1)^(ceil(Int, k/2)+1)*(1im*sqrt(2))^k*(-2)^(-k/2)/4^(k+1)*brac(k-1,alpha)*sum(repeat(reshape(g[k,1:(n+1) ], (1,1,n+1) ), outer=[2,2,1]).*Ts[:,:,(n+1):-1:1],dims=3)
                end
            end
        end
    end
