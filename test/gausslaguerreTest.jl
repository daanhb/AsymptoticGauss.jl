...
function gausslaguerreTest( n::Int, alpha::Float64=0.0, method::AbstractString="default",  qm::Float64=1.0, m::Int=1, T::Int=ceil(Int, 34/log(n) ) )
...
function laguerreGW( n::Int, alpha::Float64 )
...
function laguerreRec( n::Int, compRepr::Bool, alpha::Float64)

    if compRepr
        # Get a heuristic for the indices where the weights are about above floatmin.
        mn = min(ceil(Int, 17*sqrt(n)), n)
    else
        mn = n
    end
    itric = min(mn, 7)
...
function  ScaledLaguerreGLR( n::Int )
...
function InteriorLaguerreGLR(n::Int, roots, ders, n1::Int)
...
function BoundaryLaguerreGLR(n::Int, xs)
...
function asyRHgen(n, compRepr, alpha, m, qm)
    ...
    T = ceil(Int, 34/log(n) ) # Heuristic for number of terms, should be scaled by the logarithm of eps(Float64) over the machine precision.
    UQ0 = getUQ(alpha, qm ,m, T)
    if compRepr
        mn = min(n,ceil(Int, exp(exp(1/m)*1.05)*n^(1-1/2/m) ))
    else
        mn = n
    end
    itric = max(ceil(Int, 3.6*n^0.188), 7)
    # Heuristics to switch between Bessel, extrapolation and Airy initial guesses.
    igatt = ceil(Int, mn + 1.31*n^0.4 - n)
...
function polyAsyRHgen(np, y, alpha, T::Int, qm, m::Int, UQ, k)
    if (qm == 1) && (m == 1)
        z = y/4/np
...
function asyBesselgen(np, z, alpha, T::Int, qm, m::Int, UQ, npb, useQ::Bool)
    if T == 1
...
function asyAirygen(np, z, alpha, T::Int, qm, m::Int, UQ, fn, useQ::Bool, xin=NaN+NaN*1im)
    d = z - 1.0 +0im
    if T == 1
...
function getV(alpha,qm,m::Int,maxOrder::Int,r)
    mo = ceil(Int, 3*maxOrder/2) + 4
    ns = 0:mo
        ...
        if r == 1
            ...
            if mod(k,2) == 1
                for n = 0:mo
                    ...
                    WV[:,:,k,n+1] = -(-1)^(ceil(Int, k/2)+1)*(1im*sqrt(2))^k*(-2+0im)^(-k/2)/4^(k+1)*brac(k-1,alpha)*sum(repeat(reshape(g[k,1:(n+1) ], (1,1,n+1) ), outer=[2,2,1]).*Ts[:,:,(n+1):-1:1],3)
                end
            else
                for n = 0:mo
                    Ts[:,:,n+1] = (alpha^2+k/2-1/4)/k*4*(n==0)*eye(2)  -2*(k-1/2)*[ OmE[n+1]   4^(-alpha)*1im*XiE[n+1]  ;   4^alpha*1im*ThE[n+1]   -OmE[n+1] ]
                    WV[:,:,k,n+1] = -(-1)^(ceil(Int, k/2)+1)*(1im*sqrt(2))^k*(-2)^(-k/2)/4^(k+1)*brac(k-1,alpha)*sum(repeat(reshape(g[k,1:(n+1) ], (1,1,n+1) ), outer=[2,2,1]).*Ts[:,:,(n+1):-1:1],3)
                end
            end
        end
    end
...
function getUQ(alpha, qm, m::Int, maxOrder::Int)
    Vr = getV(alpha, qm, m, maxOrder, 1)
    Vl = getV(alpha, qm, m, maxOrder, -1)
    UQ = (1+1im)*zeros(2,2,maxOrder-1,ceil(Int,3*maxOrder/2)+2, 4)
    for kt = 0:(maxOrder-2)
        # Uright(:,:,(maxOrder-1)+1,:) will not be used later on because first term in expansions is without U's
        for mt = 0:(ceil(Int,3*(kt+1)/2)-1)
            UQ[:,:,kt+1,mt+1,1] = Vr[:,:,kt+1,ceil(Int,3*(kt+1)/2)-mt]
            for j = 0:(kt-1)
                for l = 0:(ceil(Int,3*(j+1)/2)-mt-1)
                    UQ[:,:,kt+1,mt+1,1] = UQ[:,:,kt+1,mt+1,1] + UQ[:,:,kt-j,l+1,3]*Vr[:,:,j+1,ceil(Int,3*(j+1)/2)-l-mt]
                end
            end
        end
        for mt = 0:(ceil(Int,(kt+1)/2)-1)
            UQ[:,:,kt+1,mt+1,2] = Vl[:,:,kt+1,ceil(Int,(kt+1)/2)-mt]
            for j= 0:(kt-1)
                for l = 0:(ceil(Int,(j+1)/2)-mt-1)
                    UQ[:,:,kt+1,mt+1,2] = UQ[:,:,kt+1,mt+1,2] + UQ[:,:,kt-j,l+1,4]*Vl[:,:,j+1,ceil(Int,(j+1)/2)-l-mt]
                end
            end
        end
        for n = 0:(ceil(Int,3*(maxOrder-kt+1)/2)-1)
            UQ[:,:,kt+1,n+1,3] = -Vr[:,:,kt+1,ceil(Int,3*(kt+1)/2)+1+n]
            UQ[:,:,kt+1,n+1,4] = -Vl[:,:,kt+1,ceil(Int,(kt+1)/2)+1+n]
            for i = 0:(ceil(Int,(kt+1)/2)-1)
                UQ[:,:,kt+1,n+1,3] = UQ[:,:,kt+1,n+1,3] + binom(-i-1,n)*UQ[:,:,kt+1,i+1,2]
            end
            for i = 0:(ceil(Int,3*(kt+1)/2)-1)
                UQ[:,:,kt+1,n+1,4] = UQ[:,:,kt+1,n+1,4] + binom(-i-1,n)*(-1.0)^(-i-1-n)*UQ[:,:,kt+1,i+1,1]
            end
            for j = 0:(kt-1)
                for l = 0:(ceil(Int, (j+1)/2)+n)
                    UQ[:,:,kt+1,n+1,4] = UQ[:,:,kt+1,n+1,4] -UQ[:,:,kt-j,l+1,4]*Vl[:,:,j+1,n-l+1+ceil(Int,(j+1)/2) ]
                end
                for l = 0:(ceil(Int, 3*(j+1)/2)+n)
                    UQ[:,:,kt+1,n+1,3] = UQ[:,:,kt+1,n+1,3] -UQ[:,:,kt-j,l+1,3]*Vr[:,:,j+1,n-l+1+ceil(Int, 3*(j+1)/2) ]
                end
            end
        end
    end
...
function laguerreExp( n::Int, compRepr::Bool, alpha::Float64, T::Int)
    ...
    itric = max(ceil(Int, sqrt(n)), 7)
    # Heuristics to switch between Bessel, extrapolation and Airy initial guesses.

    iair = floor(Int,0.9*n)
    ...
    # A third option is to improve the initial approximations for k starting from ichebSer = Int(round(n/10-0.45*alpha+0.3)) with barycentric interpolation on a chebyshev grid as
...
#    t[ichebSer+1:end] = t[ichebSer+1:end].*asum
    ...
    if ( minimum(x) < 0.0 ) || ( maximum(x) > 4*n + 2*alpha + 2 ) ||  ( (minimum(diff(x)) <= 0.0) && (T == ceil(Int, 34/log(n) )) && (n <= 1e5) ) || (minimum(w) < 0.0) # Higher errors when too few terms: when switching from bulk to air and avoid error for extremely large n: previously (n <= 5e6)
